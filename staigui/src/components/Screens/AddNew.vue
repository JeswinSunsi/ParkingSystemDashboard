...
async function addNewCar() {
	try {
		await axios.get(`https://cwapi.jeswinsunsi.repl.co/v1/add/${vehicletype.value}/${id.value}/${color.value}/${name.value}/${email.value}`);
		subscribeToChange();
		email.value = name.value = color.value = vehicletype.value = id.value = "";
	} catch (error) {
		console.error(error);
	}
}
